...
const loadOwners = () =>
  (process.env.OWNER || '').split(',').map(v => v.trim()).filter(Boolean);
...
const loadSudo = () => {
  ...
  // Include OWNER numbers in the combined list so isOwner works in groups
  // where fromMe is always false (bot's own echoes are blocked at index.js level)
...
module.exports = async (sock, m) => {
  ...
  const ownerList = loadOwners();
  // isOwner: fromMe (PM self-command) OR number matches OWNER env OR in sudo list
  const isOwner    = m.fromMe || isSudo(sender, ownerList) || isSudo(sender, sudoList);
  const isSudoUser = !m.fromMe && !isSudo(sender, ownerList) && isSudo(sender, sudoList);
