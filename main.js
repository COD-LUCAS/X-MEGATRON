...
module.exports = async (sock, m) => {
  ...
  // Ban check — if this chat (PM or group) is banned, bot ignores everything silently.
  // Exception: owner can always use .unban to lift a ban from inside the banned chat.
  try {
    const { isBanned } = require('./plugins/ban');
    if (isBanned(m.chat)) {
      const isOwner = m.fromMe;
      if (!isOwner) return;
      // Owner in banned chat — only allow .unban command through
      ...
      const cmd = hasPrefix ? rawBody.slice(rawBody.startsWith(prefixes.find(p => rawBody.startsWith(p))) ? prefixes.find(p => rawBody.startsWith(p)).length : 1).trim().split(/\s+/)[0]?.toLowerCase() : '';
      if (cmd !== 'unban') return;
    }
  } catch (_) {}
