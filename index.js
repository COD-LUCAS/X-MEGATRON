    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      try {
        ...
        if (mek.key.fromMe) {
          ...
          // Allow sticker messages — owner may send a sticker to trigger a bond
          const isSticker = msgType === 'stickerMessage';

          if (!isSticker) {
            // For non-sticker fromMe: must have a body starting with a prefix
            const rawBody = (
          ...
          // fromMe passes (command or sticker) — skip rate limit
