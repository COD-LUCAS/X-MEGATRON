...
// ── Module-level — survives reconnects ────────────────────
...
const initSession = async (sessionDir) => {
  ...
  if (!fs.existsSync(creds)) {
    if (!envId.includes('~')) {
      log.error('SESSION_ID must be: xmegatron~<pastebin_id>');
      process.exit(1);
    }
    ...
    try {
      ...
      if (!res.data || typeof res.data !== 'object') {
        log.error('Bad session data from Pastebin');
        process.exit(1);
      }
      fs.writeFileSync(creds, JSON.stringify(res.data, null, 2));
      log.success('Session saved');
    } catch (e) {
      log.error('Session download failed: ' + e.message);
      process.exit(1);
    }
  }
};
...
const clientstart = async () => {
  ...
  sock.ev.on('connection.update', ({ connection, lastDisconnect }) => {
    ...
    if (connection === 'close') {
      ...
      log.warn(`Reconnecting (code: ${code})...`);
  ...
  // Get configured prefixes once
  const getPfx = () =>
    (process.env.LIST_PREFIX || process.env.PREFIX || '.')
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    try {
      ...
      //
      // CASES:
      //   1. Bot number types .cmd in group/pm  → fromMe=true, body starts with prefix → ALLOW
      //   2. Bot sends a sticker bond trigger    → fromMe=true, mtype=stickerMessage   → ALLOW
      //   3. Bot's reply echoes back             → fromMe=true, body=_Pong!_ no prefix → DROP
      //   4. Bot sends media (no caption)        → fromMe=true, no body               → DROP
      //
      // Key: stickers ALWAYS pass (needed for bond trigger from bot number)
      //      text ONLY passes if it starts with a known prefix
      //      everything else is dropped to prevent blank message loops
      if (raw.key.fromMe) {
        ...
        if (!isSticker) {
          ...
          // No body at all = silent echo → drop
          if (!body) return;

          // Has body but no prefix = bot's own reply echoing back → drop
          const pfx = getPfx();
          if (!pfx.some(p => body.startsWith(p))) return;
        }

        // fromMe command or sticker — skip rate limit, process immediately
        try {
