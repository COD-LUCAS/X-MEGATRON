  ...
  try {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      try {
        ...
        // Block fromMe ONLY when it is a silent bot echo (no real text body).
        // fromMe=true with a real command prefix = owner using bot number → ALLOW.
        // fromMe=true with no body = bot's own sent message echoed back → BLOCK.
        if (mek.key.fromMe) {
          const rawBody = (
