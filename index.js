  ...
  try {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      try {
        ...
        // FIX 1: Block bot's own sent messages — prevents infinite loop spam
        if (mek.key.fromMe) return;

        // FIX 2: Dedup — skip already-processed message IDs
        ...
        // FIX 3: Unwrap ephemeral wrapper safely
        mek.message = mek.message.ephemeralMessage?.message || mek.message;

        // FIX 4: Drop silent system/protocol message types
        const msgType = Object.keys(mek.message)[0];
        if (SILENT_TYPES.has(msgType)) return;

        // FIX 5: Drop group stub events (join/leave/promote/demote)
        if (mek.messageStubType) return;

        // FIX 6: Rate limit — 1 message per second per sender+chat
