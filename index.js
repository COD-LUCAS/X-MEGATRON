  ...
  try {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      try {
        ...
        // Drop all silent system/protocol message types that produce no real body.
        // These are the sole cause of blank messages being sent in groups.
        const SILENT_TYPES = new Set([
          'protocolMessage',        // delivery receipts, revokes
          'senderKeyDistributionMessage', // key sync
          'reactionMessage',        // emoji reactions
          'pollUpdateMessage',      // poll votes
          'groupV2Change',          // group setting changes
          'groupNotification',      // join/leave banners (legacy)
          'liveLocationMessage',    // live location updates
          'callLogMesssage',        // missed call logs
          'messageStubType',        // stub system events
          'stickerSyncRmrMessage',  // sticker sync
          'keepInChatMessage',      // keep-in-chat pin
        ]);

        const msgType = Object.keys(mek.message)[0];
        if (SILENT_TYPES.has(msgType)) return;

        // Also drop if messageStubType is set (group events: join/leave/promote etc.)
