 ...
 * Data store for ALL group protection settings.
 * Saves to: database/group_data.json
 * Required by: antispam.js, antibadword.js (library)
 */
...
const DEFAULTS = { antilink: {}, antibadword: {}, antispam: {}, antitag: {}, warnings: {} };
...
// ── ANTILINK ──────────────────────────────────────────────────────────
const setAntilink    = (gid, enabled, action = 'delete') => { const d = load(); d.antilink[gid] = { enabled, action }; return save(d); };
...
// ── ANTIBADWORD ───────────────────────────────────────────────────────
const setAntibadword    = (gid, enabled, action = 'delete') => { const d = load(); const ex = d.antibadword[gid] || { words: [] }; d.antibadword[gid] = { enabled, action, words: ex.words }; return save(d); };
...
const removeBadword     = (gid, word) => { const d = load(); const words = d.antibadword[gid]?.words || []; const idx = words.indexOf(word); if (idx === -1) return false; words.splice(idx, 1); return save(d); };
const getBadwords       = (gid) => { const d = load(); return d.antibadword[gid]?.words || []; };

// ── ANTISPAM ──────────────────────────────────────────────────────────
const setAntispam    = (gid, enabled, limit = 5) => { const d = load(); d.antispam[gid] = { enabled, limit }; return save(d); };
...
// ── ANTITAG ───────────────────────────────────────────────────────────
const setAntitag    = (gid, enabled, action = 'delete') => { const d = load(); d.antitag[gid] = { enabled, action }; return save(d); };
const getAntitag    = (gid) => { const d = load(); return d.antitag[gid] || null; };
const removeAntitag = (gid) => { const d = load(); delete d.antitag[gid]; return save(d); };
...
const getWarnings      = (gid, uid) => { const d = load(); return d.warnings[gid]?.[uid] || 0; };
