...
// ── Install plugin from code string ──────────────────────────────────
...
module.exports = {
  ...
  owner:    true,
  category: 'owner',
  desc:     'Eval JS code or install plugin. Also triggered by > prefix.',
  usage:    '.eval <code>  |  .code (reply to plugin code)  |  > <code>',

  async execute(sock, m, ctx) {
    const { command, text, reply } = ctx;
    if (!ctx.isOwner) return;

    // ── .code — install plugin from replied message or inline ────
    if (command === 'code') {
      // Get code from: quoted message → inline text
      const code = (
        m.quoted?.body?.trim() ||
        m.quoted?.text?.trim() ||
      ...
      if (!code) return reply(
        `_usage:_\n` +
        `_1. Reply to a message containing plugin code with_ *.code*\n` +
        `_2. Paste code inline:_ *.code <plugin code>*`
      );

      return installPlugin(code, sock, m, ctx);
    }

    // ── .eval / .exec — run JS code ──────────────────────────────
    const code = text?.trim();
    if (!code) return reply('_provide code to evaluate_');

    // If it looks like a plugin (has module.exports), install it instead
